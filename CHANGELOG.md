...
- 对外名称由「grok2api 结合版」改为 **LocalMiniDrama 修改版**；Windows 安装包文件名去掉 `grok2api` 中缀。
...
- **画布工作台整改**：缺图/缺视频画出空槽并可就地点生成；详情改到右侧 Inspector；顶栏工作流默认折叠；首尾帧生图与尾帧衔接与列表共用同一套逻辑；工作流可跳过已有媒体、失败后续跑，整组重跑改为后端任务（关页不断）；本集灯箱、工作流分组框、全能词流式、快捷键、布局撤销、合成本集入口。
...
- 全部集数下非当前集收成可展开桩节点；缩放快捷键与右上角 − / + / 百分比一致。
...
- **画布缩放按钮**：画布右上角新增独立的 − / + 缩放控件，显示当前倍率，点击百分比可恢复 100%，旁侧提供「适应画布」

### 修复

- **剧本生成因流式静默超时失败**：推理模型（如 grok-4.6）会先输出思考过程、再长时间静默后才吐正文，实测停顿可达 120 秒，而 `generateText` 把静默超时硬编码为 60 秒，导致任务在正文到达前就被中断，剧本不入库、界面下方始终为空。现将默认静默超时放宽到 120 秒，新增 `silence_timeout_ms` 选项，剧本生成与分镜生成分别放宽到 180 秒，并支持用 `config.yaml` 的 `ai.story_generation_silence_timeout_ms` 覆盖
- **画布模式滚轮无法缩放**：`DramaCanvas.vue` 上的 `:pan-on-scroll="true"` 会让滚轮走 Vue Flow 的平移分支，滚轮缩放被完全遮蔽（只剩 Ctrl+滚轮可用），而缩放按钮又位于左下角、被「快速新建」浮动工具条压住，所以画布看起来完全不能缩放。现改为 `:pan-on-scroll="false"` + `:zoom-on-scroll="true"`，平移仍走中键/右键拖拽；缩放范围由 0.08×～2× 放宽到 0.05×～4×；右上角新增独立 − / + 按钮（原 Vue Flow 控件被浮动工具条挡住，已替换）
- **画布节点面板挡住滚轮**：四个节点面板（分镜/媒体/素材/剧本）在根元素上带了 `nowheel` 并 `@wheel.stop`，导致面板覆盖的大片区域滚轮失效。现将 `nowheel` 收窄到真正需要滚动的内部控件（文本域、摘要、音视频），面板其余区域滚轮可正常缩放画布
